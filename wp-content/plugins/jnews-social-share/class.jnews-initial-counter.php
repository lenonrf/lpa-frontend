...
Class JNews_Initial_Counter
{
    ...
    public function get_total_fake_view($total = null, $post_id, $range = 'all')
    {
        ...
        $view_initial_value = apply_filters('jnews_view_initial_counter', jnews_get_option('single_view_initial_value', 0), $post_id);
        $total = (int) $total + (int) $view_initial_value;
    ...
    {
        $fake_share = apply_filters('jnews_share_initial_counter', $this->fake_share($post_id), $post_id);
        $totalshare = (int) $total + (int) $fake_share;
        return $totalshare;
    }
